...
//            NotaImageBuilder(order, kostumRepository).generateAndShareReceipt(this@NotaActivity)
    ...
    private fun loadData() {
        lifecycleScope.launch {
            ...
            val user = pinjaman!!.id_peminjam.let {
                pelangganApiService.getPelangganById(it).body()!!.data
            }
...
//            sbHeader.append("Faktur: ").append(pinjaman.id).append("\n")
            ...
            sbHeader.append("Tanggal Awal: ")
                .append(formatDate.format(parseDate.parse(pinjaman.tgl_pinjam)!!)).append("\n")
            sbHeader.append("Tanggal Akhir: ")
                .append(formatDate.format(parseDate.parse(pinjaman.tgl_kembali)!!)).append("\n")
            ...
            binding.tvHargaAwal.text = "Rp" + barangDipinjam?.harga.toString()
            binding.tvMingguPinjam.text = weeksDifference.toString() + " minggu"
            binding.tvTotalHarga.text = "Rp" + totalHarga.toString()
        }
    }
}